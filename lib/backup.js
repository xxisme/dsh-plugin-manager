 ...
 * @param {string} dir 目标父目录（备份会建在这里面）
 */
function uniqueBackupDirName(dir) {
...
/**
 * 创建一次 profile 配置文件备份
 * 目录结构：backups/profile/<profile>/<timestamp>/
...
export function backupProfile(dataDir, dshHome, profileName, manual = false) {
  ...
  const profileBackupDir = path.join(backupRoot(dataDir), 'profile', profileName);
  const dir = path.join(profileBackupDir, uniqueBackupDirName(profileBackupDir));
...
/**
 * 列出所有备份（扫描 backups/profile/<profile>/<ts>/）
 * 返回：{ name, dir, meta, profile }
 */
export function listBackups(dataDir) {
  const root = path.join(backupRoot(dataDir), 'profile');
  const out = [];
  if (!fs.existsSync(root)) return out;
  for (const profileEnt of fs.readdirSync(root, { withFileTypes: true })) {
    if (!profileEnt.isDirectory()) continue;
    const pd = path.join(root, profileEnt.name);
    for (const ent of fs.readdirSync(pd, { withFileTypes: true })) {
      if (!ent.isDirectory()) continue;
      const dir = path.join(pd, ent.name);
      let meta = null;
      const metaPath = path.join(dir, 'meta.json');
      if (fs.existsSync(metaPath)) {
        try { meta = JSON.parse(fs.readFileSync(metaPath, 'utf-8')); } catch { /* ignore */ }
      }
      out.push({ name: ent.name, dir, meta, profile: profileEnt.name });
    }
  }
  return out.sort((a, b) => b.name.localeCompare(a.name)); // 倒序，新的在前
}
...
/**
 * 清理超过 keepLimit 的旧备份（按 profile 分组，每个 profile 保留前 N 个，避免多 profile 互相挤掉）
 */
export function cleanupOldBackups(dataDir, keepLimit = 10) {
  const list = listBackups(dataDir);
  const byProfile = {};
  for (const b of list) {
    (byProfile[b.profile || '?'] = byProfile[b.profile || '?'] || []).push(b);
  }
  let removed = 0;
  for (const arr of Object.values(byProfile)) {
    for (const b of arr.slice(keepLimit)) {
      try { fs.rmSync(b.dir, { recursive: true, force: true }); removed += 1; } catch { /* ignore */ }
    }
  }
  return removed;
}
