...
import { parseImporterVersions } from './updater.js';
...
export function listPlugins(dshHome, profileName, dataDir) {
  ...
  // 从 lockfile 读实际已解析版本——这是「主面板」要显示给用户看的版本号。
  // （之前 listPlugins 返回的 version 字段是 package.json 里的 specifier 如 `^0.1.2`，
  //   pnpm update 不会改 specifier，所以面板上看到的一直是 `^0.1.2`，看起来没更新。
  //   实际 lockfile 里可能已经是 0.1.4 了。2026-08-17 修。）
  const profDir = profileDir(dshHome, profileName);
  const lockPath = path.join(profDir, 'pnpm-lock.yaml');
  let resolved = {};
  if (fs.existsSync(lockPath)) {
    try { resolved = parseImporterVersions(fs.readFileSync(lockPath, 'utf8')); }
    catch { /* lockfile 坏了——继续走 specifier 兑底 */ }
  }
  ...
  const result = bundles.map((bundleName) => {
    ...
    return {
      ...
      // version 优先用 lockfile 的 resolved version；没有才回退到 specifier。
      // specifier 单独存供 UI 需要时（比如 tooltip 或 tooltip）使用。
      version: (isCore ? null : resolved[bundleName]) || pkgInfo?.version || null,
      specifier: pkgInfo?.version || null,
  ...
  const extras = Object.entries(deps)
    ...
    .map(([name, spec]) => {
      ...
      return {
        id: name,
        source: info.source,
        version: resolved[name] || info.version,  // 同样优先 lockfile
