...
function buildRestoreCommands(spec, opts = {}) {
  ...
  if (spec.strategy === 'pointer') {
    if (spec.installKind === 'registry') {
      ...
      // 用原始 specifier（github:owner/repo）还原，commit 由 lock 锁定
      cmds.push(`pnpm add "${spec.lockSpecifier}"`);
    } else {
...
async function restorePluginsExec(specs, pluginsRoot, exec) {
  ...
  for (const spec of specs) {
    try {
      if (spec.strategy === 'pointer') {
        // pointer：从上游重拉（registry 锁版本，github/git 用原始 specifier）
        let specStr;
        if (spec.installKind === 'registry') {
          specStr = spec.localVersion ? `${spec.pkg}@${spec.localVersion}` : spec.pkg;
        } else if (['git', 'github', 'github-tarball'].includes(spec.installKind)) {
          specStr = spec.lockSpecifier || spec.pkg; // 如 github:owner/repo
        } else {
...
export { restorePlugins, buildRestoreCommands, restoreBlobOne, copyDirFlat, restorePluginsExec };
