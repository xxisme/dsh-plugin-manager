...
// ── lockfile 哈希（仅 importers 段，排除 packages 与 settings）──
// 用途是验证「dsh plugin update 之后 lockfile 到底改没改」，不关心 packages 段的连带重写。
export function lockfileImportersHash(lockText) {
  if (!lockText) return null;
  // importers 段一直延伸到 packages: 头。V9/V10 格式都适用。
  const m = lockText.match(/^importers:[\s\S]*?(?=^packages:|\Z)/m);
  const section = m ? m[0] : lockText;
...
// 导出供 dsh-profile.js 使用，让「已装插件」面板显示 lockfile 里的实际版本，而不是 package.json 里的 specifier。
...
// 导出供 dsh-profile.js 使用（从底部 export 列表出）。
...
export { checkUpdates, parseGithubDeps, parseNpmDeps, parseImporterVersions, fetchLatestCommit, fetchUpstreamHistory, findLocalInHistory };
